...
def load_saved_predictor(model_path, msa_one_hot=None) :
	...
	def _load_predictor_func(sequence_input, saved_model=saved_model) :
		...
		#msa_one_hot_var = Lambda(lambda x: K.stop_gradient(K.concatenate([K.expand_dims(x, axis=1), K.zeros((1, 1, K.shape(x)[1], 1, 1))], axis=-2)))(sequence_input) #Input(tensor=K.constant(msa_one_hot))
		msa_one_hot_var = Lambda(lambda x: K.concatenate([K.expand_dims(x, axis=1), K.zeros((1, 1, K.shape(x)[1], 1, 1))], axis=-2))(sequence_input) #Input(tensor=K.constant(msa_one_hot))
